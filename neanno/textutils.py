...
def extract_entities_from_nerded_text(text, allowed_entities=None):
    """ Returns all entities and their positions from a "nerded" text, eg. This is an (example entity|NE SOME_ENTITY_TYPE)."""
    # get "empty" result without having entity positions
    result = (re.sub("\((?P<text>.+?)\|NE .+?\)", "\g<text>", text), {"entities": []})
    # add entity positions
    working_text = text
    while True:
        find_entities_pattern = "\((?P<text>.+?)\|NE (?P<label>.+?)\)"
        ...
        if match is not None:
            ...
            working_text = re.sub("\(.+?\|NE .+?\)", entity_text, working_text, 1)
        else:
            break
    return result
